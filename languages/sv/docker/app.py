    ...
    def decorated_function(*args, **kwargs):
        ...
        if is_guest_user(session['username']):
            ...
            if not getattr(f, '_guest_allowed', False):
                flash('Denna funktion är inte tillgänglig för gästanvändare.', 'danger')
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if 'username' not in session:
            flash('Var god logga in först.', 'warning')
            return redirect(url_for('login'))
        ...
        if not user or not user.get('admin'):
            flash('Admin-åtkomst krävs.', 'danger')
            return redirect(url_for('dashboard'))
...
def guest_allowed(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'username' not in session:
            flash('Vänligen logga in först.', 'warning')
            return redirect(url_for('login'))
...
def change_email():
    ...
    for user in users:
        ...
        flash('Hittar inte användaren.', 'danger')
        return redirect(url_for('dashboard'))

    # Save the updated data back to the JSON file
    save_users(users)

    flash('E-postadressen har uppdaterats.', 'success')
    return redirect(url_for('dashboard'))
...
def auth():
    ...
    if state != saved_state:
        flash("Auktorisering misslyckades: ogiltigt tillstånd.", "danger")
        return redirect(url_for("login"))
    ...
    try:
        ...
        flash("OIDC-auktorisation misslyckades.", "danger")
        return redirect(url_for("login"))
    ...
    try:
        ...
        flash("Det gick inte att tolka användarinformation.", "danger")
        return redirect(url_for("login"))
    ...
    if user_in_db:
        ...
        flash("Inloggning lyckades med OIDC!", "login_success")
        return redirect(url_for("dashboard"))
    ...
    if os.getenv("ENABLE_AUTO_REGISTRATION", "false").lower() == "true":
        ...
        flash("Ny profil skapad. Vänligen fyll i din profilinställning.", "info")
        return redirect(url_for("setup_profile"))  # Redirect to profile setup page

    flash("Användaren hittades inte och automatisk registrering är inaktiverad.", "danger")
    return redirect(url_for("login"))
...
def setup_profile():
    ...
    if user.get("avatar"):
        flash("Konfiguration av profil krävs inte. Avatar har redan angetts.", "info")
        return redirect(url_for("dashboard"))
    ...
    if request.method == "POST":
        ...
        flash("Profilkonfigurationen slutförd!", "success")
        return redirect(url_for("dashboard"))
...
def index():
    ...
    try:
        ...
        flash(f"Fel vid läsning av users.json: {e}", 'danger')
        return redirect(url_for('setup'))
...
def login():
    ...
    login_message = read_env_variable("LOGIN_PAGE_MESSAGE") or "Inget konto? Kontakta en familjemedlem för att skapa ett konto."
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for user in users:
                if user['username'].lower() == input_username:
                    ...
                    if verify_password(user['password'], password):
                        ...
                        flash('Inloggning lyckades!', 'login_success')
                        return redirect(url_for('dashboard'))
                    else:
                        flash('Fel lösenord', 'login_error')
            ...
            flash(f"Fel vid läsning av users.json: {e}", 'login_error')
...
def register():
    ...
    if not os.getenv('ENABLE_SELF_REGISTRATION', 'false').lower() == 'true':
        flash('Självregistrering är inte aktiverad. Vänligen kontakta en administratör.', 'danger')
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if joining_code and submitted_joining_code != joining_code:
            flash('Ogiltig anslutningskod. Vänligen kontrollera med din familj.', 'danger')
            return render_template('register.html', joining_code=joining_code)
        
        # Validate required input
        if not username or not password or not full_name:
            flash('Vänligen fyll i alla obligatoriska fält.', 'danger')
            return render_template('register.html', joining_code=joining_code)
        ...
        if any(user['username'].lower() == username for user in users):
            flash('Användarnamnet finns redan. Vänligen välj en annan.', 'danger')
            return render_template('register.html', joining_code=joining_code)
        
        # Check for duplicate email only if email is provided
        if email and any(user.get('email', '').lower() == email.lower() for user in users):
            flash('E-postadress redan registrerad. Använd en annan e-post eller kontakta support.', 'danger')
            return render_template('register.html', joining_code=joining_code)
...
def add2():
    ...
    if not current_user_data:
        flash("Nuvarande användare hittades inte.", "danger")
        return redirect(url_for('dashboard'))
...
def add_idea(selected_user_id):
    ...
    if request.method == 'POST':
        ...
        flash(f'Idé "{name}" tillagd för användaren {user} av {added_by}!', 'success')
        return redirect(url_for('user_gift_ideas', selected_user_id=user))
    imgenabled = os.getenv('IMGENABLED', 'true').lower() == 'true'
...
def delete_idea(idea_id):
    ...
    if idea:
        ...
        if idea['added_by'] == current_user_username or idea['user_id'] == current_user_username:
            ...
            flash('Du är inte behörig att radera detta förslaget.', 'danger')
    else:
        flash('Förslaget hittades inte', 'danger')
...
def dashboard():
    ...
    if not current_user:
        flash('Användardata hittades inte', 'danger')
        return redirect(url_for('login'))
...
def change_password():
    ...
    if new_password != confirm_password:
        flash('Nya lösenordet och bekräftelselösenordet stämmer inte överens', 'danger')
        return redirect(url_for('dashboard'))
...
def mark_as_bought(idea_id):
    ...
    if idea:
        if not idea['bought_by']:
            ...
            flash(f'Markerade "{idea["gift_name"]}" som köpt!', 'success')
            ...
            flash(f'"{idea["gift_name"]}" har redan köpts av {idea["bought_by"]}.', 'warning')
    else:
        flash('Presentförslag hittades inte', 'danger')
...
def mark_as_not_bought(idea_id):
    ...
    if idea:
        ...
        if idea['bought_by'] == session['username']:
            ...
            flash(f'Markerade "{idea["gift_name"]}" som inte köpt.', 'success')

            # Save the updated gift ideas back to the JSON file
            save_gift_ideas(gift_ideas_data)  # Save the updated list
        else:
            flash(f'Du köpte inte "{idea["gift_name"]}", så du kan inte markera det som inte köpt.', 'danger')
    else:
        flash('Presentförslag hittades inte', 'danger')
...
def my_ideas():
    ...
    if not my_gift_ideas:
        flash('Du har inte lagt till några presentförslag.', 'info')
        return redirect(url_for('noidea'))
...
def add_user():
    if request.method == 'POST':
        ...
        flash('Användaren har lagts till!', 'success')
    ...
    if idea:
        ...
        if idea['added_by'] == current_user_username or idea['user_id'] == current_user_username:
            if request.method == 'POST':
                ...
                flash('Förslaget har uppdaterats!', 'success')
...
def secret_santa():
    ...
    if request.method == 'POST':
        ...
        if pool_name_to_delete:
            ...
            if not pool_exists:
                flash(f'Grupp "{pool_name_to_delete}" finns inte.', 'error')
            else:
                ...
                save_users(users)
                flash(f'Grupp "{pool_name_to_delete}" har tagits bort!', 'success')
            ...
            if not pool_name:
                flash('Namn på tomtegrupp krävs!', 'error')
                return redirect(url_for('secret_santa'))

            if len(selected_participants) < 2:
                flash('Du behöver minst 2 deltagare för hemliga tomten!', 'error')
                return redirect(url_for('secret_santa'))
...
def secret_santa_assignments():
    ...
    if not assigned_users:
        flash("Du har inga hemliga tomte uppdrag ännu.", "error")
        return redirect(url_for('secret_santa'))
...
def edit_email_settings():
    if request.method == 'POST':
        ...
        try:
            ...
            flash("E-postinställningarna har uppdaterats!", "success")
        except Exception as e:
    ...
    if request.method == 'POST':
        ...
        if new_message:
            try:
                ...
                flash("Inloggningsmeddelandet har uppdaterats!", "success")
            except Exception as e:
                flash(f"Fel vid uppdatering av inloggningsmeddelande: {e}", "danger")
        else:
            flash("Meddelande kan inte vara tomt.", "danger")

        return redirect(url_for('edit_login_message'))

    # Fetch the current login message directly from .env
    current_message = read_env_variable("LOGIN_PAGE_MESSAGE") or "Inget konto? Kontakta en familjemedlem för att skapa ett konto."
...
def delete_old_gift_ideas_page():
    ...
    if request.method == 'POST':
        try:
            ...
            flash(f"{deleted_count} presentförslag äldre än tröskeln har tagits bort.", "success")

            # Render the result to the template
            return render_template('delete_old_gift_ideas.html', deleted_count=deleted_count)

        except Exception as e:
            flash(f"Fel vid borttagning av gamla presentförslag: {e}", "danger")
    ...
    if request.method == 'POST':
        ...
        if new_days:
            try:
                ...
                flash(f"Antalet dagar att ta bort gamla presentförslag har uppdaterats till {new_days}.", "success")
                return redirect(url_for('change_delete_days'))

            except ValueError:
                flash("Ogiltigt antal dagar angivna. Vänligen ange ett giltigt nummer.", "danger")
        else:
            flash("Ange ett värde för antalet dagar.", "danger")
...
def setup():
    ...
    if users_data:  # If there are any users in the file
        flash('Användare är redan konfigurerade. Inställningssidan är inte tillgänglig.', 'info')
    ...
    if request.method == 'POST':
        ...
        flash('Administratörskonto har skapats. Konfigurera miljövariablerna.', 'success')
...
def setupenv():
    ...
    if not referer or '/setupadmin' not in referer:
        flash('Du måste komma åt denna sida från setupadmin rutt.', 'error')
    ...
    if request.method == 'POST':
        ...
        flash('Miljövariabler sparades!', 'success')
...
def setup_oidc():
    if request.method == 'POST':
        ...
        if missing_fields:
            flash(f"Obligatoriska fält saknas: {', '.join(missing_fields)}", 'danger')
        ...
        try:
            ...
            flash('OIDC-inställningar sparades!', 'success')
            return redirect(url_for('setup_oidc'))  # Redirect to home page after saving
        except Exception as e:
            flash(f"Fel vid sparande av OIDC-inställningar: {e}", "danger")
            return render_template('setup_oidc.html', current_values=oidc_env_variables)
    ...
    try:
        ...
        flash("Miljöfil hittades inte. Skapar en ny.", "warning")
...
def restart_container():
    try:
        ...
        flash("Det gick inte att starta om behållaren. Försök igen senare.", "error")
...
def manage_groups():
    ...
    if request.method == 'POST':
        ...
        if new_group_name:
            ...
            flash('Ny grupp tillagd!', 'success')
...
def update_group_assignments():
    ...
    flash('Gruppuppgifter uppdaterades!', 'success')
    return redirect(url_for('manage_groups'))
...
def manage_guest_users():
    ...
    if request.method == 'POST':
        ...
        save_users(users)
        flash('Gästanvändare har skapats!', 'success')
...
def delete_guest_user(username):
    ...
    if guest_user:
        ...
        flash(f'Gästanvändaren {username} har raderats! Alla privata grupper borttagna och {deleted_count} köpta gåvoidéer borttagna.', 'success')
    else:
        flash('Kunde inte hitta gästanvändare.', 'danger')
...
def guest_login():
    ...
    for user in users:
        if user.get('guest') and verify_password(user['password'], password):
            session['username'] = user['username']
            flash('Gästinloggning lyckades!', 'login_success')
            return redirect(url_for('dashboard'))
    
    flash('Ogiltigt gästlösenord!', 'login_error')
    return redirect(url_for('login'))
...
def update_currency_settings():
    ...
    flash('Valutainställningar uppdaterades! Vänligen starta om för att se ändringar', 'success')
    return redirect(url_for('setup_advanced'))
