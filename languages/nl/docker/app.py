...
def login():
    ...
    login_message = read_env_variable("LOGIN_PAGE_MESSAGE") or "Geen account? Neem contact op met een familielid om een account aan te maken."
...
def mark_as_not_bought(idea_id):
    ...
    if idea:
        ...
        if idea['bought_by'] == session['username']:
            ...
            flash(f'"{idea["gift_name"]}" gemarkeerd als niet gekocht.', 'success')
...
def restart_container():
    try:
        ...
        flash("Container herstarten mislukt. Probeer het later nog eens.", "error")
        return redirect(url_for('dashboard'))  # Redirect on error
    except Exception as e:
        print("General error:", str(e))
        flash(f"Er is een onverwachte fout opgetreden: {str(e)}", "error")
