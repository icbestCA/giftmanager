...
with open(app.config['IDEAS_FILE'], 'r') as ideas_file:
...
for idea in gift_ideas_data:
    if 'date_bought' in idea:
        date_bought = datetime.strptime(
            idea['date_bought'], '%Y-%m-%d %H:%M:%S')
...
with open(app.config['IDEAS_FILE'], 'w') as ideas_file:
...
print(f"Ideas older than {
      threshold_days} days have been removed. Total removed: {removed_count}")
